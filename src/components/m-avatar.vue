    ...
    <view class="oper-wrapper" :style="{display: styleDisplay}">
      <view class="oper">
        ...
        <view class="clr-wrapper" v-else>
          <slider class="my-slider" @change="fColorChange" block-size="25" value="0" min="-100" max="100" activeColor="red" backgroundColor="green" block-color="grey" show-value></slider>
            ...
            success: r => {
              ...
              if (!this.hasSel) {
                ...
                if (this.selWidth && this.selHeight) {
                  ...
                  util.showToastError("裁剪框的宽或高没有设置");
                  return;
                }
                this.selStyle = style;
              }
      ...
      if (!this.prvImgData) {
        if (
          !(this.prvImgData = await this.fGetImgData().catch(res => {
            util.showToastError("error_read");
          }))
        )
          return;
